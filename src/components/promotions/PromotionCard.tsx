...
export const PromotionCard = ({
  ...
  return <motion.div initial={{
    ...
    duration: 0.5,
    delay: index * 0.1
  }} className="group relative overflow-hidden rounded-2xl bg-white border" style={{
    minHeight: 450
  }}>
      <div className="relative aspect-[3/2] overflow-hidden rounded-t-2xl">
        ...
        <div className="absolute left-0 right-0 bottom-3 flex justify-center">
          <div className="bg-black/70 rounded px-2 py-1 flex items-center gap-1">
            {Array.from({
            length: 5
          }).map((_, idx) => <Star key={idx} className={`w-4 h-4 ${idx < rating ? "text-yellow-400" : "text-gray-400"}`} fill={idx < rating ? "#facc15" : "none"} />)}
          </div>
        </div>
      </div>
      <div className="px-5 pt-5 pb-6 flex flex-col h-[calc(100%-170px)] py-[45px]">
        ...
        <div className="mt-auto">
          <a href={bookingUrl} target="_blank" rel="noopener noreferrer">
            <Button variant="outline" className="w-full bg-[#005488] text-white border-none hover:bg-[#004070] transition-all">
              View Deal
            </Button>
          </a>
        </div>
